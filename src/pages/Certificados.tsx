...
export default function Certificados() {
  ...
  const certificadosPorBusca = certificados.filter(c => {
  ...
  const ativos    = certificadosPorBusca.filter(c => getStatus(c.data_vencimento).id === "ativo").length;
  const aExpirar  = certificadosPorBusca.filter(c => getStatus(c.data_vencimento).id === "a_expirar").length;
  const vencidos  = certificadosPorBusca.filter(c => getStatus(c.data_vencimento).id === "vencido").length;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Duplicata: mesmo CNPJ/CPF (nome pode repetir)
    const isDuplicate = cnpjNorm.length > 0 && certificados.some(c => {
      if (c.id === editingId) return false;
      return ((c as any).cnpj || "").replace(/\D/g, "") === cnpjNorm;
    });

    if (isDuplicate) {
      toast({
        title: "Certificado duplicado",
        description: `Já existe um certificado com o CNPJ/CPF ${form.cnpj}. Atualize o existente.`,
        variant: "destructive",
      });
      return;
    }
              ...
              {certificadosFiltrados.length > 0 ? certificadosFiltrados.map(cert => {
                ...
                return (
                  <TableRow key={cert.id}>
                    ...
                    <TableCell>
                      {(() => {
                        const notifs = emailNotificacoes.filter(n => n.referencia_id === cert.id);
